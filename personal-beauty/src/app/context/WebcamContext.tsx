...
export const WebcamProvider: React.FC<{ children: React.ReactNode }> = ({
    children,
}) => {
    ...
    const lastPositionBeforeFist = useRef<{ x: number; y: number } | null>(
        null
    );
    ...
    const detectGesture = useCallback((landmarks: any[]) => {
        ...
        const distanceIndex = Math.hypot(
    ...
    const detectFull = useCallback(
        (landmarks: any[], isIndexRaised: boolean) => {
        ...
        try {
            ...
            mediaStream.getVideoTracks().forEach((track) => {
                track.addEventListener("ended", async () => {
                    console.warn(
                        "[WebcamProvider] Video track ended. Restarting..."
                    );
                    await restartStream();
                });
                track.addEventListener("mute", async () => {
                    console.warn(
                        "[WebcamProvider] Video track muted. Restarting..."
                    );
                    await restartStream();
                });
                track.addEventListener("inactive", async () => {
                    console.warn(
                        "[WebcamProvider] Video track inactive. Restarting..."
                    );
                    await restartStream();
                });
            });
        ...
        workerRef.current.onmessage = (e: MessageEvent) => {
            ...
            if (type === "detectionResult") {
                if (results?.hand?.landmarks?.length > 0) {
                    ...
                    if (isIndexRaised) {
                        if (!indexRaiseStartTime.current)
                            indexRaiseStartTime.current = now;
    ...
    useEffect(() => {
        ...
        const unusedModels = allModels.filter(
            (m) => !requiredModels.includes(m)
        );
    ...
    useEffect(() => {
        ...
        const detect = async () => {
            ...
            if (isHandDetectionEnabled) {
                if (isIndexFingerRaised) minInterval = 33;
                else minInterval = 66;
            } else {
                minInterval = 125;
            }
            ...
            if (video.readyState < 4 || video.paused) {
                console.warn(
                    "[WebcamProvider] Video not ready or paused, skipping frame."
                );
                animationFrameId.current = requestAnimationFrame(detect);
                return;
            }
            ...
            try {
                ...
                if (imageBitmap.width === 0 || imageBitmap.height === 0) {
                    console.warn(
                        "[WebcamProvider] Empty ImageBitmap, skipping frame."
                    );
                    animationFrameId.current = requestAnimationFrame(detect);
                    return;
                }
